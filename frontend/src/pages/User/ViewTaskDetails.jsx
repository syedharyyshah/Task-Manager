...
const ViewTaskDetails = () => {
  ...
  const updateTodoChecklist = async (index) => {
    ...
    try {
      ...
      } else if (checklistResponse.status === 200 && checklistResponse.data?.task) {
        setTask(checklistResponse.data.task);
      } else {
        setTask({
          ...task,
          todoChecklist: updatedChecklist,
  ...
  if (!task) {
    return (
      <DashboardLauout activeMenu="My Tasks">
        <div className="mt-5">Task not found</div>
      </DashboardLauout>
    );
  }
    ...
    <DashboardLauout activeMenu="My Tasks">
      <div className="mt-5">
        <div className="grid grid-cols-1 md:grid-cols-4 mt-4">
          <div className="form-card dark:text-gray-50 col-span-3">
            <div className="flex items-center justify-between">
              <h2 className="text-base md:text-xl font-medium">{task.title}</h2>
              <div className={`text-[13px] px-4 py-2 rounded font-medium ${getStatusTagColor(task.status)}`}>
                {task.status}
              </div>
            </div>
            ...
            <div className="mt-2">
              ...
              {task.todoChecklist?.map((item, index) => (
                <div key={`todo_${index}`} className="flex items-center gap-3 p-3">
                  <input
                    type="checkbox"
